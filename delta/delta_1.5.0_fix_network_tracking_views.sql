...
CREATE MATERIALIZED VIEW qgep_od.vw_network_segment AS
 ...
 (
   ...
   (
     ...
     ST_Force2D( ST_LineFromMultiPoint( ST_Collect(wn_from.situation_geometry, rp_from.situation_geometry ) ) ) AS progression_geometry
     FROM qgep_od.reach
     ...
     SELECT
       ...
       ST_Force2D( ST_LineFromMultiPoint( ST_Collect(rp_to.situation_geometry, wn_to.situation_geometry ) ) ) AS progression_geometry
     FROM qgep_od.reach
