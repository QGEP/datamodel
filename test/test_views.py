...
class TestViews(unittest.TestCase, DbTestBase):
    ...
    def test_vw_qgep_wastewater_structure(self):
        row = {
                'identifier': '20',
                'ws_type': 'manhole',
                'situation_geometry': '01040000A0080800000100000001010000800000000020D6434100000000804F32410000000000000000', # SELECT ST_SetSRID(ST_GeomFromText('MULTIPOINTZ(2600000 1200000 0)'), 2056)
                'co_material': 5355,
                'wn_backflow_level': decimal.Decimal('100.000')
        }

        expected_row = copy.deepcopy(row)
        expected_row['situation_geometry'] = '01040000A0080800000100000001010000800000000020D6434100000000804F32410000000000000000' # SELECT ST_SetSRID(ST_GeomFromText('MULTIPOINT(2600000 1200000)'), 2056)
