...
CREATE MATERIALIZED VIEW qgep_od.vw_network_node AS
 ...
 (
   SELECT
     ...
     NULL AS description,
     situation_geometry::geometry(Point,2056) AS detail_geometry,
...
ALTER TABLE qgep_od.reach_point ALTER COLUMN situation_geometry TYPE geometry('POINTZ', 2056) USING ST_SetSRID( ST_MakePoint( ST_X(situation_geometry), ST_Y(situation_geometry), COALESCE(level,'NaN') ), 2056);
...
ALTER TABLE qgep_od.wastewater_node ALTER COLUMN situation_geometry TYPE geometry('POINTZ', 2056) USING ST_SetSRID( ST_MakePoint( ST_X(situation_geometry), ST_Y(situation_geometry), COALESCE(bottom_level,'NaN') ), 2056);
-- in quarantine we do not add the level to the geometry because this will be done on insert on table - so it's always 'NaN'
ALTER TABLE qgep_import.manhole_quarantine ALTER COLUMN situation_geometry TYPE geometry('POINTZ', 2056) USING ST_SetSRID( ST_MakePoint( ST_X(situation_geometry), ST_Y(situation_geometry), 'NaN' ), 2056);
...
ALTER TABLE qgep_od.cover ALTER COLUMN situation_geometry TYPE geometry('POINTZ', 2056) USING ST_SetSRID( ST_MakePoint( ST_X(situation_geometry), ST_Y(situation_geometry), COALESCE(level,'NaN') ), 2056);
...
BEGIN
  CASE
    WHEN TG_OP = 'INSERT' THEN
      NEW.situation_geometry = ST_SetSRID( ST_MakePoint( ST_X(NEW.situation_geometry), ST_Y(NEW.situation_geometry), COALESCE(NEW.bottom_level,'NaN') ), 2056);
...
BEGIN
  CASE
    WHEN TG_OP = 'INSERT' THEN
      NEW.situation_geometry = ST_SetSRID( ST_MakePoint( ST_X(NEW.situation_geometry), ST_Y(NEW.situation_geometry), COALESCE(NEW.level,'NaN') ), 2056);
    WHEN TG_OP = 'UPDATE' THEN
