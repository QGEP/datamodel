...
-- 2. Rename Tables
ALTER TABLE qgep.od_overflow_characteristic RENAME TO qgep.od_overflow_char;
ALTER TABLE qgep.od_hydraulic_characteristic_data RENAME TO qgep.od_hydraulic_char_data;
...
ALTER TABLE qgep.od_hq_relation ADD CONSTRAINT rel_hq_relation_overflow_char FOREIGN KEY (fk_overflow_characteristic) REFERENCES qgep.od_overflow_char(obj_id) ON UPDATE CASCADE ON DELETE cascade;

ALTER TABLE qgep.od_hydraulic_char_data ADD CONSTRAINT rel_hydraulic_char_data_overflow_char FOREIGN KEY (fk_overflow_characteristic) REFERENCES qgep.od_overflow_char(obj_id) ON UPDATE CASCADE ON DELETE set null;

ALTER TABLE qgep.od_overflow ADD CONSTRAINT rel_overflow_overflow_char FOREIGN KEY (fk_overflow_characteristic) REFERENCES qgep.od_overflow_char(obj_id) ON UPDATE CASCADE ON DELETE set null;
